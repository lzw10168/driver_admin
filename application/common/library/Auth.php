...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'score','money'];
    ...
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
        if ($username && $username != '微信用户' && User::getByUsername($username)) {
            $this->setError('Username already exist');
            return false;
        }
        if ($username && $username != '微信用户' && User::getByNickname($username)) {
    ...
    public function delete($user_id)
    {
        ...
        try {
            ...
            Db::name('ddrive_user_token')->where('user_id', $user_id)->delete();
